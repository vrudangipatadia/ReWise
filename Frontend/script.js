...
async function login() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Gettign specific user's saved flashcards
            loadCards(); 
...
// Profile Settings overlay popup options (Username/Password changes)
async function openAccountAction(type) {
    ...
    // Backup check: If the app forgots who is logged in, get it back from the UI text label
    if (!currentUser) {
        const nameEl = document.getElementById('full-username');
...
// Clear the user session data and back to login screen
function logout() {
    currentUser = null;
    document.getElementById('main-app').style.display = 'none';
    document.getElementById('auth-box').style.display = 'block';
    document.getElementById('l-pass').value = ''; // Clear password field just to be safe
}
...
async function addCard() {
    ...
    // New card shoudl appear at the front of list so it shows up instantly
    allCards.unshift(newCard);
    renderCards(allCards);
    
    // Clears out input boxes so they're ready for the next flashcard
...
function openEdit(id, q, a, t) {
    editingId = id; // Lock in the ID we are changing
